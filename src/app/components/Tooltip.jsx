...
export default function Tooltip({ text, position = 'top', children }) {
  ...
  return (
    <div className="group relative inline-block">
      {children}
      <div
        className={`pointer-events-none absolute ${posClasses[position]} z-10 w-max max-w-xs whitespace-nowrap rounded bg-gray-800 px-2 py-1 text-xs text-white opacity-0 transition-opacity duration-150 group-hover:opacity-100`}
      >
        {text}
      </div>
    </div>
