...
// Enhanced MarkdownRenderer for clearer hierarchy and book-friendly formatting
export default function MarkdownRenderer({ text, fontSize, theme }) {
  const proseClass =
    theme === 'dark'
  ...
  return (
    <div style={{ fontSize: `${fontSize}px` }} className={`${proseClass} leading-relaxed tracking-wide`}>
      <ReactMarkdown
        ...
        components={{
          h1: props => (
            <h1 className="text-4xl font-bold uppercase text-center mt-8 mb-4" {...props} />
          ),
          h2: props => (
            <h2 className="text-3xl font-semibold italic mt-6 mb-3" {...props} />
          ),
          h3: props => (
